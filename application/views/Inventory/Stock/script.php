<script>
	...
	$table = $('#stock_table').DataTable( {
        ...
        "createdRow": function ( row, data, index ) {
			...
			if(data['stock_sum'] <=0)
			{
				$('td', row).eq(5).html('<center><button type="button" class="btn btn-block btn-danger">Out of Stock</button></center>');
			}
			else if(data['stock_sum'] > 0 && data['stock_sum'] < data['min_stock'])
			{
				$('td', row).eq(5).html('<center><button type="button" class="btn btn-block btn-warning">Reached Below</button></center>');
			}
			else
			{
				$('td', row).eq(5).html('<center><button type="button" class="btn btn-block btn-success">Available</button></center>');
			}
