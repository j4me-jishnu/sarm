    ...
    function getInvoiceNumber(el,event,item){
     ...
     //alert(item.purchase_id);
        if(item.purchase_id){
            ...
            var purchase_qty = item.product_purchase_quantity;
            var sale_qty = item.purchase_return_qty;
            var avalible = parseFloat(purchase_qty) - parseFloat(sale_qty);
            $("#purchase_qty").val(avalible);
            $("#purchase_Qty").val(avalible);
...
$('#ex_productname').rcm_autoComplete('<?php echo base_url();?>index.php/common/getProductList',$productList,param,getexProductName);
...
//alert(return_quantity);
       var options = {
		 ...
		 var return_quantity = parseFloat(return_quantity);
       var purchase_Qty = parseFloat(purchase_Qty);
