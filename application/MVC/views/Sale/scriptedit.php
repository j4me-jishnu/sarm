	...
	function getProductName(el,event,item){
       console.log(item);
        if(item.product_id){
            ...
            var diff = parseFloat(item.product_purchase_quantity) - parseFloat(item.purchase_return_qty);
...
function AddSale(){
  ...
  var sq = $("#Sale_qty").val();
  var cq = $("#checkquantity").val();
  var sq = parseFloat(sq);
  var cq = parseFloat(cq);
  var options1 = {
         'title': 'Error',
         'style': 'error',
  ...
  if(!/^[0-9]+$/.test(sale_qty) || !/^[0-9]+$/.test(sale_discount) || !/^[0-9]+$/.test(sale_rte)){ 
//     $('#EditSale').modal('hide');
  }
  else if(sale_qty ==='' || sale_discount ==='' || sale_rte ==='')
  {
